...
class OverviewHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final storeName = CacheHelper.getData(key: 'name') ?? 'Nile Weavers';
    ...
    if (storeName.isNotEmpty) {
      ...
      if (parts.length > 1) {
        ...
        initials = storeName.substring(0, storeName.length > 1 ? 2 : 1).toUpperCase();
      }
    }
    ...
    return BlocBuilder<SellerCubit, SellerState>(
      builder: (context, state) {
        ...
        final revenue = cubit.dashboardData?.totalRevenue ?? 0.0;
        final activeOrders = cubit.dashboardData?.activeOrdersCount ?? 0;
